...
Private Function SeriesHasFormula(seriesToCheck As series) As Boolean
    'this is a function to trap errors that occur when reading the formula of a bad series
    On Error Resume Next
    Dim seriesFormula As String
    seriesFormula = seriesToCheck.Formula
    On Error GoTo 0
    
    If seriesFormula <> "" Then
        SeriesHasFormula = True
    Else
        SeriesHasFormula = False
    End If
